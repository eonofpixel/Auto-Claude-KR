...
export function initializeGit(projectPath: string): InitializationResult {
  ...
  try {
    ...
    // Step 2: Create/update .gitignore to exclude problematic files
    // Windows reserved names (nul, con, aux, etc.) and embedded repos can cause issues
    const gitignorePath = path.join(projectPath, '.gitignore');
    const defaultIgnoreEntries = [
      '# Auto-generated gitignore entries',
      '.auto-claude/',
      '',
      '# Windows reserved device names (can cause git errors)',
      'nul',
      'con',
      'aux',
      'prn',
    ...
    // Check if .gitignore exists and add our entries if needed
    ...
    for (const entry of defaultIgnoreEntries) {
    ...
    if (entriesToAdd.length > 0) {
      const appendContent = (gitignoreContent && !gitignoreContent.endsWith('\n') ? '\n' : '') +
        entriesToAdd.join('\n') + '\n';
      if (existsSync(gitignorePath)) {
        appendFileSync(gitignorePath, appendContent);
      } else {
        writeFileSync(gitignorePath, defaultIgnoreEntries.join('\n') + '\n');
      }
      debug('Updated .gitignore with default entries');
    }
    ...
    // Step 4: If there are untracked/modified files, add and commit them
    if (statusOutput || !status.hasCommits) {
      debug('Adding files and creating initial commit');

      // Add files using "git add ." which respects .gitignore
      // Use try-catch to handle partial failures gracefully
      try {
        ...
        // If add fails, try adding with --ignore-errors
        debug('git add failed, trying with --ignore-errors');
        try {
          execFileSync(git, ['add', '.', '--ignore-errors'], {
            cwd: projectPath,
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe']
          });
        } catch {
          // Even ignore-errors failed, but we can still try to commit
          debug('git add --ignore-errors also failed, will try empty commit');
        }
      }

      // Create initial commit (allow-empty ensures we get a commit even if nothing staged)
...
export function initializeProject(projectPath: string): InitializationResult {
  ...
  if (existsSync(dotAutoBuildPath)) {
    ...
    // Return success if already initialized - this is not an error condition
    return {
      success: true
    };
  }
