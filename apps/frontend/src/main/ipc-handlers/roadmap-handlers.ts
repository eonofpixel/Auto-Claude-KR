...
export function registerRoadmapHandlers(
  ...
  ipcMain.handle(
    ...
    async (_, projectId: string): Promise<IPCResult<{
      isRunning: boolean;
      currentProgress?: { phase: string; progress: number; message: string };
    }>> => {
      const isRunning = agentManager.isRoadmapRunning(projectId);
      const currentProgress = isRunning ? agentManager.getRoadmapProgress(projectId) : null;
      debugLog("[Roadmap Handler] Get status:", { projectId, isRunning, currentProgress });
      return {
        success: true,
        data: {
          isRunning,
