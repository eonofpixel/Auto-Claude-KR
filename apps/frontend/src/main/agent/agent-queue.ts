...
/**
 * Stored progress state for running roadmap generation
 */
...
export class AgentQueueManager {
  ...
  // Store current roadmap progress by projectId for status queries
  private roadmapProgressMap: Map<string, RoadmapProgressState> = new Map();
  ...
  async startRoadmapGeneration(
    ...
    // Add language for AI-generated content (ko, fr, en)
    if (config?.language) {
  ...
  private async spawnRoadmapProcess(
    ...
    // Initialize progress tracking in map
    this.roadmapProgressMap.set(projectId, {
      phase: progressPhase,
      progress: progressPercent,
    ...
    childProcess.stdout?.on('data', (data: Buffer) => {
      ...
      // Store current progress for status queries
      const statusMessage = formatStatusMessage(log);
      this.roadmapProgressMap.set(projectId, {
        phase: progressPhase,
        progress: progressPercent,
    ...
    childProcess.on('exit', (code: number | null) => {
      ...
      if (wasIntentionallyStopped) {
        ...
        this.state.clearKilledSpawn(spawnId);
        // Clear progress tracking for stopped process
        this.roadmapProgressMap.delete(projectId);
        // Note: Don't call deleteProcess here - killProcess() already deleted it.
        // A new process with the same projectId may have been started.
        return;
      }
      ...
      const storedProjectPath = processInfo?.projectPath;
      this.state.deleteProcess(projectId);
      // Clear progress tracking on completion/error
      this.roadmapProgressMap.delete(projectId);
  ...
  /**
   * Get current roadmap progress for a project
   * Returns null if no roadmap generation is running
   */
  getRoadmapProgress(projectId: string): RoadmapProgressState | null {
    return this.roadmapProgressMap.get(projectId) || null;
  }
}
