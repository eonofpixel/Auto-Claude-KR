...
import { GitSetupModal } from './components/GitSetupModal';
...
import type { Task, Project, ColorTheme, GitStatus } from '../shared/types';
...
export function App() {
  ...
  // Git setup state (shown when project needs git initialization)
  const [showGitSetup, setShowGitSetup] = useState(false);
  const [gitSetupProject, setGitSetupProject] = useState<Project | null>(null);
  const [gitSetupStatus, setGitSetupStatus] = useState<GitStatus | null>(null);
  ...
  const handleInitialize = async () => {
    ...
    try {
      ...
      const gitStatusResult = await window.electronAPI.checkGitStatus(pendingProject.path);
      console.log('[InitDialog] Git status check result:', gitStatusResult);

      if (gitStatusResult.success && gitStatusResult.data) {
        const gitStatus = gitStatusResult.data;

        // If git is not ready, show GitSetupModal instead
        if (!gitStatus.isGitRepo || !gitStatus.hasCommits) {
          console.log('[InitDialog] Git not ready, showing GitSetupModal');
          setIsInitializing(false);
          setShowInitDialog(false);

          // Show GitSetupModal
          setGitSetupProject(pendingProject);
          setGitSetupStatus(gitStatus);
          setShowGitSetup(true);
          return;
        }
      }

      // Git is ready, proceed with initialization
      ...
      if (result?.success) {
        ...
        const errorMessage = result?.error || t('initialize.initFailedDescription');
      ...
      const errorMessage = error instanceof Error ? error.message : t('common:errors.unexpectedError');
      setInitError(errorMessage);
      setIsInitializing(false);
    }
  };
  ...
  // Called when git setup is completed (git init + initial commit)
  const handleGitSetupComplete = async () => {
    console.log('[GitSetup] Git setup completed, proceeding with Auto Claude initialization');
    const project = gitSetupProject;

    // Close git setup modal
    setShowGitSetup(false);
    setGitSetupProject(null);
    setGitSetupStatus(null);

    if (project) {
      // Now try to initialize Auto Claude
      setPendingProject(project);
      setShowInitDialog(true);
    }
  };
  ...
  return (
    <ViewStateProvider>
      ...
      <div className="flex h-screen bg-background">
        ...
        {/* Git Setup Modal - shows when project needs git initialization */}
        <GitSetupModal
          open={showGitSetup}
          onOpenChange={setShowGitSetup}
          project={gitSetupProject}
          gitStatus={gitSetupStatus}
          onGitInitialized={handleGitSetupComplete}
        />
