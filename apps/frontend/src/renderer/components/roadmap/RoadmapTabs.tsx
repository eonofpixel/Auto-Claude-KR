  ...
  return (
    <Tabs value={activeTab} onValueChange={onTabChange} className="h-full flex flex-col">
      <TabsList className="shrink-0 mx-4 mt-4">
        <TabsTrigger value="kanban">{t('roadmapTabs.kanban')}</TabsTrigger>
        <TabsTrigger value="phases">{t('roadmapTabs.phases')}</TabsTrigger>
        <TabsTrigger value="features">{t('roadmapTabs.allFeatures')}</TabsTrigger>
        <TabsTrigger value="priorities">{t('roadmapTabs.byPriority')}</TabsTrigger>
      </TabsList>
              ...
              <Card key={priority} className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  <Badge variant="outline" className={ROADMAP_PRIORITY_COLORS[priority]}>
                    {t(ROADMAP_PRIORITY_LABEL_KEYS[priority])}
                  </Badge>
                  <span className="text-sm text-muted-foreground">
                    {t('roadmapFeature.featuresCount', { count: features.length })}
                  </span>
                </div>
                  ...
                  {features.map((feature: RoadmapFeature) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        >
                          {t(`roadmapFeature.complexity.${feature.complexity}`)}
                        </Badge>
                        <Badge
                          variant="outline"
                          className={`text-xs ${ROADMAP_IMPACT_COLORS[feature.impact]}`}
                        >
                          {t(`roadmapFeature.impact.${feature.impact}`)}
                        </Badge>
                        {hasCompetitorInsight(feature) && (
                          <Badge variant="outline" className="text-xs text-primary border-primary/50">
                            <TrendingUp className="h-3 w-3 mr-1" />
                            {t('roadmapFeature.insight')}
                          </Badge>
                        )}
                      </div>
                    </div>
