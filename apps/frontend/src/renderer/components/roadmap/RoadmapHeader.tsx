...
import { ROADMAP_PRIORITY_COLORS, ROADMAP_PRIORITY_LABEL_KEYS } from '../../../shared/constants';
              ...
              <Tooltip>
                ...
                <TooltipContent className="max-w-md">
                  <div className="space-y-2">
                    <div className="font-semibold">{t('roadmap.viewAnalysisTooltip')}</div>
                    <div className="text-sm text-muted-foreground">
                      {t('roadmap.analyzedCompetitors', { count: competitorAnalysis.competitors.length, painPoints: competitorAnalysis.competitors.reduce((sum, c) => sum + c.painPoints.length, 0) })}
                    </div>
                  </div>
                </TooltipContent>
              </Tooltip>
      ...
      <div className="mt-4 flex items-center gap-6">
        ...
        <div className="flex items-center gap-1">
          {Object.entries(stats.byPriority).map(([priority, count]) => (
            ...
            >
              {count} {t(ROADMAP_PRIORITY_LABEL_KEYS[priority])}
            </Badge>
          ))}
        </div>
      </div>
