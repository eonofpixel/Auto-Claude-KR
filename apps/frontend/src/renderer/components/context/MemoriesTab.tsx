...
// Translation key mapping for filter labels
const filterLabelKeys: Record<FilterCategory, string> = {
  all: 'context.memories.filterAll',
  pr: 'context.memories.filterPR',
  sessions: 'context.memories.filterSessions',
  codebase: 'context.memories.filterCodebase',
  patterns: 'context.memories.filterPatterns',
  gotchas: 'context.memories.filterGotchas'
};
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <Input
              placeholder={t('common:placeholders.searchMemories')}
          ...
          {searchResults.length > 0 && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                {t('context.memories.resultsFound', { count: searchResults.length })}
              </p>
                  ...
                  <CardContent className="pt-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t('context.memories.score', { score: result.score.toFixed(2) })}
                      </span>
                    </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {t('context.memories.memoriesCount', { filtered: filteredMemories.length, total: recentMemories.length })}
            </span>
          </div>
            ...
            {(Object.keys(memoryFilterCategories) as FilterCategory[]).map((category) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{t(filterLabelKeys[category])}</span>
                  {count > 0 && (
                    <Badge
                      variant="secondary"
                      className={cn(
          ...
          {!memoriesLoading && filteredMemories.length === 0 && recentMemories.length === 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {t('context.memories.noMemoriesYet')}
              </p>
            </div>
          )}
          ...
          {!memoriesLoading && filteredMemories.length === 0 && recentMemories.length > 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <Brain className="h-10 w-10 text-muted-foreground mb-3" />
              <p className="text-sm text-muted-foreground">
                {t('context.memories.noMatchingMemories')}
              </p>
              <Button
                variant="link"
                size="sm"
                ...
                className="mt-2"
              >
                {t('context.memories.showAllMemories')}
              </Button>
            </div>
          )}
