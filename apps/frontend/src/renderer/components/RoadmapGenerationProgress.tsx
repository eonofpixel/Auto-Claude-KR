...
// Phase display configuration (translation keys)
const PHASE_CONFIG_KEYS: Record<
  GenerationPhase,
  {
    labelKey: string;
...
> = {
  analyzing: {
    labelKey: 'roadmapGeneration.phases.analyzing',
    descriptionKey: 'roadmapGeneration.phases.analyzingDesc',
    icon: Search,
    color: 'bg-amber-500',
    bgColor: 'bg-amber-500/20',
  },
  discovering: {
    labelKey: 'roadmapGeneration.phases.discovering',
    descriptionKey: 'roadmapGeneration.phases.discoveringDesc',
    icon: Users,
    color: 'bg-info',
    bgColor: 'bg-info/20',
  },
  generating: {
    labelKey: 'roadmapGeneration.phases.generating',
    descriptionKey: 'roadmapGeneration.phases.generatingDesc',
    icon: Sparkles,
    color: 'bg-primary',
    bgColor: 'bg-primary/20',
  },
  complete: {
    labelKey: 'roadmapGeneration.phases.complete',
    descriptionKey: 'roadmapGeneration.phases.completeDesc',
    icon: CheckCircle2,
    color: 'bg-success',
    bgColor: 'bg-success/20',
  },
  error: {
    labelKey: 'roadmapGeneration.phases.error',
    descriptionKey: 'roadmapGeneration.phases.errorDesc',
    icon: AlertCircle,
...
const STEP_PHASE_KEYS: { key: GenerationPhase; labelKey: string }[] = [
  { key: 'analyzing', labelKey: 'roadmapGeneration.steps.analyze' },
  { key: 'discovering', labelKey: 'roadmapGeneration.steps.discover' },
  { key: 'generating', labelKey: 'roadmapGeneration.steps.generate' },
];
...
function PhaseStepsIndicator({
  ...
  return (
    <div className="flex items-center justify-center gap-1 mt-4">
      {STEP_PHASE_KEYS.map((phase, index) => {
          ...
          <div key={phase.key} className="flex items-center">
            ...
            {index < STEP_PHASE_KEYS.length - 1 && (
              <div
                className={cn(
                  'w-4 h-px mx-1',
                  getPhaseState(STEP_PHASE_KEYS[index + 1].key) !== 'pending'
                    ? 'bg-success/50'
                    : 'bg-border'
                )}
              />
...
export function RoadmapGenerationProgress({
  ...
  const config = PHASE_CONFIG_KEYS[phase];
      ...
      {isActivePhase && onStop && (
        <div className="flex justify-end mb-2">
          <Tooltip>
            ...
            <TooltipContent>{t('roadmapGeneration.stopGeneration')}</TooltipContent>
          </Tooltip>
        </div>
      )}
      ...
      {isActivePhase && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs text-muted-foreground">{t('roadmapGeneration.progress')}</span>
