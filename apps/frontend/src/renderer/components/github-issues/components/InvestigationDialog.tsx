      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('github.issues.createTaskFromIssue')}
          </DialogTitle>
          <DialogDescription>
            {selectedIssue && (
              <span>
        ...
        {investigationStatus.phase === 'idle' ? (
          <div className="space-y-4 flex-1 min-h-0 flex flex-col">
            <p className="text-sm text-muted-foreground">
              {t('github.issues.createTaskDescription')}
            </p>
            ...
            {loadingComments ? (
              ...
              <div className="rounded-lg bg-destructive/10 border border-destructive/30 p-4">
                <p className="text-sm text-destructive font-medium">{t('github.issues.failedToLoadComments')}</p>
              ...
              <div className="space-y-2 flex-1 min-h-0 flex flex-col">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-medium flex items-center gap-2">
                    ...
                    {t('github.issues.selectCommentsToInclude', { selected: selectedCommentIds.length, total: comments.length })}
                  </h4>
                  <Button
                    variant="ghost"
                    size="sm"
                    ...
                    className="text-xs"
                  >
                    {selectedCommentIds.length === comments.length ? t('github.issues.deselectAll') : t('github.issues.selectAll')}
                  </Button>
                </div>
                <ScrollArea
              ...
              <div className="rounded-lg border border-border bg-muted/30 p-4">
                <h4 className="text-sm font-medium mb-2">{t('github.issues.taskWillInclude')}</h4>
                <ul className="text-sm text-muted-foreground space-y-1">
                  <li>• {t('github.issues.includeTitle')}</li>
                  <li>• {t('github.issues.includeLink')}</li>
                  <li>• {t('github.issues.includeLabels')}</li>
                  <li>• {t('github.issues.noComments')}</li>
                </ul>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {investigationStatus.phase === 'complete' && (
              <div className="rounded-lg bg-success/10 border border-success/30 p-3 flex items-center gap-2 text-sm text-success">
                ...
                {t('github.issues.taskCreatedSuccess')}
              </div>
            )}
          </div>
        )}
