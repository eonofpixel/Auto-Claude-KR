    ...
    <div className="shrink-0 p-4 border-b border-border">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-xs">
            {t('github.issues.openCount', { count: openIssuesCount })}
          </Badge>
          <Button
            variant="ghost"
            size="icon"
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {t('github.issues.analyzeAndGroup')}
                </Button>
              </TooltipTrigger>
              <TooltipContent side="bottom" className="max-w-xs">
                <p>{t('github.issues.analyzeTooltip')}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
          ...
          <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 border border-border">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="max-w-xs">
                  <p>{t('github.issues.autoFixTooltip')}</p>
                  {autoFixRunning && autoFixProcessing !== undefined && autoFixProcessing > 0 && (
                    <p className="mt-1 text-primary">{t('github.issues.processingIssues', { count: autoFixProcessing })}</p>
                  )}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder={t('github.searchIssuesPlaceholder')}
            value={searchQuery}
        ...
        <Select value={filterState} onValueChange={onFilterChange}>
          ...
          <SelectContent>
            <SelectItem value="open">{t('github.issues.filterOpen')}</SelectItem>
            <SelectItem value="closed">{t('github.issues.filterClosed')}</SelectItem>
            <SelectItem value="all">{t('github.issues.filterAll')}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    </div>
