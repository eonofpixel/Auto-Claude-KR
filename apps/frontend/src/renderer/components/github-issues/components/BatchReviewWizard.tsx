...
export function BatchReviewWizard({
  ...
  const renderIntro = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('github.issues.analyzeDialogTitle')}</h3>
        <p className="text-sm text-muted-foreground max-w-md">
          {t('github.issues.analyzeDialogDescription')}
        </p>
      </div>
      {analysisError && (
  ...
  const renderAnalyzing = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('github.issues.analyzingIssues')}</h3>
        <p className="text-sm text-muted-foreground">
          {analysisProgress?.message || t('github.issues.computingSimilarity')}
        </p>
      </div>
      <div className="w-full max-w-md">
        <Progress value={analysisProgress?.progress ?? 0} />
        <p className="text-xs text-center text-muted-foreground mt-2">
          {t('github.issues.percentComplete', { percent: analysisProgress?.progress ?? 0 })}
        </p>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col h-[60vh]">
        ...
        <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg mb-4">
          <div className="flex items-center gap-4 text-sm">
            <span>
              <strong>{t('github.issues.issuesAnalyzed', { count: totalIssues })}</strong>
            </span>
            <span className="text-muted-foreground">|</span>
            <span>
              <strong>{t('github.issues.batchesProposed', { count: proposedBatches.length })}</strong>
            </span>
            <span className="text-muted-foreground">|</span>
            <span>
              <strong>{t('github.issues.singleIssues', { count: singleIssues.length })}</strong>
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={selectAllBatches}>
              {t('github.issues.selectAll')}
            </Button>
            <Button variant="ghost" size="sm" onClick={deselectAllBatches}>
              {t('github.issues.deselectAll')}
            </Button>
          </div>
        </div>
          ...
          {singleIssues.length > 0 && (
            <div className="mt-6">
              <h4 className="text-sm font-medium text-muted-foreground mb-2">
                {t('github.issues.singleIssuesNotGrouped')}
              </h4>
              <div className="grid grid-cols-2 gap-2">
                ...
                {singleIssues.length > 10 && (
                  <div className="p-2 text-sm text-muted-foreground">
                    {t('github.issues.andMore', { count: singleIssues.length - 10 })}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center justify-between pt-4 mt-4 border-t border-border">
          <div className="text-sm text-muted-foreground">
            {t('github.issues.batchSelected', { count: selectedCount, issues: totalIssuesInSelected })}
            {selectedSingleIssueNumbers.size > 0 && (
              <> {t('github.issues.plusSingleIssues', { count: selectedSingleIssueNumbers.size })}</>
            )}
          </div>
        </div>
      </div>
  ...
  const renderApproving = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      <Loader2 className="h-12 w-12 text-primary animate-spin" />
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('github.issues.creatingBatches')}</h3>
        <p className="text-sm text-muted-foreground">
          {t('github.issues.settingUpBatches')}
        </p>
      </div>
    </div>
  );
  ...
  const renderDone = () => (
    <div className="flex flex-col items-center justify-center py-8 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">{t('github.issues.batchesCreated')}</h3>
        <p className="text-sm text-muted-foreground">
          {t('github.issues.batchesReadyForProcessing')}
        </p>
      </div>
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('github.issues.analyzeDialogTitle')}
          </DialogTitle>
          <DialogDescription>
            {step === 'intro' && t('github.issues.introDescription')}
            {step === 'analyzing' && t('github.issues.analyzingDescription')}
            {step === 'review' && t('github.issues.reviewDescription')}
            {step === 'approving' && t('github.issues.approvingDescription')}
            {step === 'done' && t('github.issues.doneDescription')}
          </DialogDescription>
        </DialogHeader>
            ...
            >
              {isApproving ? (
                ...
                <>
                  ...
                  {t('github.issues.approveAndCreate', { count: selectedBatchIds.size + selectedSingleIssueNumbers.size })}
                </>
              )}
            </Button>
          </DialogFooter>
          ...
          <div className="flex items-center justify-between">
            <CollapsibleTrigger className="flex items-center gap-2 hover:underline">
              ...
              <span className="font-medium text-sm">
                {batch.theme || t('github.issues.batch', { number: index + 1 })}
              </span>
            </CollapsibleTrigger>
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="text-xs">
                ...
                {t('github.issues.issueCount', { count: batch.issueCount })}
              </Badge>
              <Badge
              ...
              {batch.issues.map((issue) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">
                    {t('github.issues.percentSimilar', { percent: Math.round(issue.similarityToPrimary * 100) })}
                  </span>
                </div>
              ))}
            </div>
