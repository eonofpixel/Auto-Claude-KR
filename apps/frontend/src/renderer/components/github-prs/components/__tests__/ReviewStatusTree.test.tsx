...
describe('ReviewStatusTree - Reviewing Status (ACS-200)', () => {
  ...
  describe('Follow-up Review in Progress', () => {
    it('should show follow-up specific steps when isReviewing with previousReviewResult', () => {
      ...
      // Should show previous review step (using translation key pattern) - count is the number of findings in previous review
      expect(screen.getByText(new RegExp(i18n.t('prReview.previousReview', { count: 1 }).replace(/[()]/g, '\\$&')))).toBeInTheDocument();

      // Should show new commits step
      expect(screen.getByText(i18n.t('prReview.newCommits', { count: 2 }))).toBeInTheDocument();
  ...
  describe('Completed Review States', () => {
    ...
    it('should show ready_for_followup status with run follow-up button', () => {
      ...
      expect(screen.getAllByText(i18n.t('prReview.readyForFollowup')).length).toBeGreaterThan(0);

      // Should show run follow-up button
      const followUpButton = screen.getByText(i18n.t('prReview.runFollowup'));
