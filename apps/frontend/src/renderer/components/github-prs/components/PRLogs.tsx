...
// Phase labels are now handled via i18n - see getPhaseLabel function
...
export function PRLogs({ prNumber, logs, isLoading, isStreaming = false }: PRLogsProps) {
  ...
  const getPhaseLabel = (phase: PRLogPhase): string => {
    return t(`common:prReview.logs.phases.${phase}`);
  };
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-muted-foreground flex items-center gap-2">
                PR #{prNumber}
                {logs.is_followup && <Badge variant="outline" className="text-xs">{t('common:prReview.logs.followUp')}</Badge>}
                {isStreaming && (
                  <Badge variant="outline" className="text-xs bg-blue-500/10 text-blue-500 border-blue-500/30 flex items-center gap-1">
                    <Loader2 className="h-2.5 w-2.5 animate-spin" />
                    {t('common:prReview.logs.live')}
                  </Badge>
                )}
              </div>
...
function PhaseLogSection({ phase, phaseLog, phaseLabel, isExpanded, onToggle, isStreaming = false, expandedAgents, onToggleAgent }: PhaseLogSectionProps) {
  const { t } = useTranslation(['common']);
    ...
    if (status === 'active' || (isStreaming && status === 'pending')) {
      return (
        <Badge variant="outline" className="text-xs bg-info/10 text-info border-info/30 flex items-center gap-1">
          ...
          {isStreaming ? t('common:prReview.logs.status.streaming') : t('common:prReview.logs.status.running')}
        </Badge>
      );
    }
    ...
    if (isStreaming && status === 'completed' && !hasEntries) {
      return (
        <Badge variant="secondary" className="text-xs text-muted-foreground">
          {t('common:prReview.logs.status.pending')}
        </Badge>
      );
    }
      ...
      case 'completed':
        return (
          <Badge variant="outline" className="text-xs bg-success/10 text-success border-success/30 flex items-center gap-1">
            ...
            {t('common:prReview.logs.status.complete')}
          </Badge>
        );
      case 'failed':
        return (
          <Badge variant="outline" className="text-xs bg-destructive/10 text-destructive border-destructive/30 flex items-center gap-1">
            ...
            {t('common:prReview.logs.status.failed')}
          </Badge>
        );
      default:
        return (
          ...
          <div className="flex items-center gap-2">
            ...
            {hasEntries && (
              <span className="text-xs text-muted-foreground">
                ({t('common:prReview.logs.entriesCount', { count: phaseLog?.entries.length })})
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
