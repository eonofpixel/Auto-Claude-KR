...
function getSourceKey(source: SDKRateLimitInfo['source']): string {
  switch (source) {
    case 'changelog': return 'rateLimit.sources.changelog';
    case 'task': return 'rateLimit.sources.task';
    case 'roadmap': return 'rateLimit.sources.roadmap';
    case 'ideation': return 'rateLimit.sources.ideation';
    case 'title-generator': return 'rateLimit.sources.titleGenerator';
    default: return 'rateLimit.sources.claudeOperation';
  }
}
...
export function SDKRateLimitModal() {
  ...
  const sourceName = t(getSourceKey(sdkRateLimitInfo.source));
        ...
        <div className="py-4 space-y-4">
          ...
          <div className="rounded-lg border border-accent/50 bg-accent/10 p-4">
            ...
            {hasMultipleProfiles ? (
              <>
                <p className="text-sm text-muted-foreground mb-3">
                  {suggestedProfile ? (
                    <>{t('rateLimit.sdk.recommended')}: <strong>{suggestedProfile.name}</strong> {t('rateLimit.sdk.hasMoreCapacity')}</>
                  ) : (
                    t('rateLimit.sdk.switchAndRetry')
                  )}
                </p>
                    ...
                    <DropdownMenuTrigger asChild>
                      <Button variant="outline" className="flex-1 justify-between">
                        <span className="truncate flex items-center gap-2">
                          {selectedProfile?.name || t('rateLimit.sdk.selectAccount')}
                    ...
                    <DropdownMenuContent align="start" className="w-[220px] bg-popover border border-border shadow-lg">
                        ...
                        >
                          <span className="truncate flex items-center gap-2">
                            ...
                            {profile.id === sdkRateLimitInfo.suggestedProfile?.id && (
                              <Star className="h-3 w-3 text-yellow-500" aria-label={t('rateLimit.sdk.recommended')} />
                            )}
                          </span>
                      ...
                      <DropdownMenuItem
                        onClick={() => {
                          ...
                          const input = document.querySelector('input[data-add-account]') as HTMLInputElement;
                          if (input) input.focus();
                        }}
                        className="flex items-center gap-2 text-muted-foreground"
                      >
                        <Plus className="h-4 w-4" />
                        {t('rateLimit.sdk.addNewAccount')}
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
              ...
              <p className="text-sm text-muted-foreground mb-3">
                {t('rateLimit.sdk.addSubscriptionForAutoSwitch')}
              </p>
            )}
            ...
            <div className={hasMultipleProfiles ? "mt-4 pt-3 border-t border-border/50" : ""}>
              <p className="text-xs text-muted-foreground mb-2">
                {hasMultipleProfiles ? t('rateLimit.sdk.addAnotherAccount') : t('rateLimit.sdk.connectAccount')}
              </p>
              <div className="flex items-center gap-2">
                <Input
                  data-add-account
                  placeholder={t('rateLimit.sdk.accountNamePlaceholder')}
          ...
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
            <h4 className="text-sm font-medium text-foreground mb-2">
              {t('rateLimit.sdk.upgradeTitle')}
            </h4>
            <p className="text-sm text-muted-foreground mb-3">
              {t('rateLimit.sdk.upgradeDescription')}
            </p>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              ...
              {t('rateLimit.sdk.upgradeSubscription')}
            </Button>
          </div>
