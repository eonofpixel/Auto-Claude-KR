...
// Phase label translation keys (use with t() from react-i18next)
const PHASE_LABEL_KEYS: Record<TaskLogPhase, string> = {
  planning: 'execution.phases.planning',
  coding: 'execution.phases.coding',
  validation: 'execution.phases.reviewing'
};
      ...
      <div className="p-4 space-y-2">
        {isLoadingLogs ? (
          ...
          <div className="text-center text-sm text-muted-foreground py-8">
            ...
            <p>{t('taskLogs.noLogs')}</p>
            <p className="text-xs mt-1">{t('taskLogs.logsWillAppear')}</p>
          </div>
        )}
      </div>
    </div>
    ...
    <Collapsible open={isExpanded} onOpenChange={onToggle}>
      <CollapsibleTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-sm">{t(PHASE_LABEL_KEYS[phase])}</span>
            {hasEntries && (
              <span className="text-xs text-muted-foreground">
                ({phaseLog?.entries.length} {phaseLog?.entries.length === 1 ? t('execution.labels.entry') : t('execution.labels.entries')})
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
      ...
      <CollapsibleContent>
        <div className="mt-1 ml-6 border-l-2 border-border pl-4 py-2 space-y-1">
          {!hasEntries ? (
            <p className="text-xs text-muted-foreground italic">{t('taskLogs.noLogs')}</p>
          ) : (
...
function LogEntry({ entry }: LogEntryProps) {
  ...
  const getToolInfo = (toolName: string) => {
    switch (toolName) {
      case 'Read':
        return { icon: FileText, label: t('taskLogs.tools.reading'), color: 'text-blue-500 bg-blue-500/10' };
      case 'Glob':
        return { icon: FolderSearch, label: t('taskLogs.tools.searchingFiles'), color: 'text-amber-500 bg-amber-500/10' };
      case 'Grep':
        return { icon: Search, label: t('taskLogs.tools.searchingCode'), color: 'text-green-500 bg-green-500/10' };
      case 'Edit':
        return { icon: Pencil, label: t('taskLogs.tools.editing'), color: 'text-purple-500 bg-purple-500/10' };
      case 'Write':
        return { icon: FileCode, label: t('taskLogs.tools.writing'), color: 'text-cyan-500 bg-cyan-500/10' };
      case 'Bash':
        return { icon: Terminal, label: t('taskLogs.tools.running'), color: 'text-orange-500 bg-orange-500/10' };
      default:
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-2">
          <div className={cn('inline-flex items-center gap-2 rounded-md px-2 py-1 text-xs', color, 'opacity-60')}>
            ...
            <span className="text-muted-foreground">{t('taskLogs.done')}</span>
          </div>
          {hasDetail && (
            <button
            ...
            >
              {isExpanded ? (
                <>
                  ...
                  <span>{t('taskLogs.hideOutput')}</span>
                </>
              ) : (
                <>
                  <ChevronRight className="h-2.5 w-2.5" />
                  <span>{t('taskLogs.showOutput')}</span>
                </>
              )}
            </button>
          )}
        ...
        {hasDetail && (
          ...
          >
            {isExpanded ? (
              <>
                <ChevronDown className="h-2.5 w-2.5" />
                <span>{t('taskLogs.less')}</span>
              </>
            ) : (
              <>
                <ChevronRight className="h-2.5 w-2.5" />
                <span>{t('taskLogs.more')}</span>
              </>
            )}
          </button>
        )}
