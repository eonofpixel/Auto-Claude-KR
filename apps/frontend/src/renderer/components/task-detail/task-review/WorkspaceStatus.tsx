      ...
      <div className="px-4 py-3 bg-muted/30 border-b border-border">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-medium text-sm text-foreground flex items-center gap-2">
            ...
            {t('taskReview:workspace.buildReadyForReview')}
          </h3>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div className="flex items-center gap-4 text-xs">
          <span className="flex items-center gap-1.5 text-muted-foreground">
            ...
            <span className="font-medium text-foreground">{worktreeStatus.filesChanged || 0}</span> {t('taskReview:workspace.files')}
          </span>
          <span className="flex items-center gap-1.5 text-muted-foreground">
            <GitCommit className="h-3.5 w-3.5" />
            <span className="font-medium text-foreground">{worktreeStatus.commitCount || 0}</span> {t('taskReview:workspace.commits')}
          </span>
        ...
        {worktreeStatus.worktreePath && (
          <div className="flex gap-2 mt-3">
            ...
            >
              ...
              {t('taskReview:workspace.openInIDE', { ide: IDE_LABELS[preferredIDE] })}
            </Button>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              ...
              {t('taskReview:workspace.openInTerminal', { terminal: TERMINAL_LABELS[preferredTerminal] })}
            </Button>
          </div>
        )}
      </div>
          ...
          <div className="flex items-start gap-2 p-2.5 rounded-lg bg-warning/10 border border-warning/20">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-warning">
                {t('taskReview:workspace.uncommittedChanges', { count: uncommittedCount })}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {t('taskReview:workspace.uncommittedChangesHint')}
              </p>
            </div>
          </div>
        )}
              ...
              {hasGitConflicts ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    <span className="text-xs text-muted-foreground ml-2">
                      {t('taskReview:workspace.aiWillResolveCount', { count: hasPathMappedMerges ? pathMappedAIMergeCount : commitsBehind, type: hasPathMappedMerges ? 'files' : 'commits' })}
                    </span>
                  </div>
                </>
              ) : !hasAIConflicts ? (
                <>
                  ...
                  <span className="text-xs text-muted-foreground ml-1">
                    {t('taskReview:workspace.filesCount', { count: mergePreview.summary.totalFiles })}
                  </span>
                </>
              ) : (
                <>
                  <AlertTriangle className="h-4 w-4 text-warning" />
                  <span className="text-sm font-medium text-warning">
                    {t('taskReview:workspace.conflictsCount', { count: mergePreview.conflicts.length })}
                  </span>
                </>
              )}
            </div>
      ...
      <div className="px-4 py-3 bg-muted/20 border-t border-border space-y-3">
        ...
        {mergePreview && (
          <label className="inline-flex items-center gap-2.5 text-sm cursor-pointer select-none px-3 py-2 rounded-lg border border-border bg-background/50 hover:bg-background/80 transition-colors">
            ...
            )}>{t('taskReview:workspace.stageOnlyOption')}</span>
          </label>
        )}
