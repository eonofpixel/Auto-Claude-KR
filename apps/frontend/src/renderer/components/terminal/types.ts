...
// Static phase config for styling (colors and icons only)
const PHASE_STYLE_CONFIG: Record<ExecutionPhase, { color: string; icon: React.ElementType }> = {
  idle: { color: 'bg-muted text-muted-foreground', icon: Circle },
  planning: { color: 'bg-info/20 text-info', icon: Search },
  coding: { color: 'bg-primary/20 text-primary', icon: Code2 },
  qa_review: { color: 'bg-warning/20 text-warning', icon: Search },
  qa_fixing: { color: 'bg-warning/20 text-warning', icon: Wrench },
  complete: { color: 'bg-success/20 text-success', icon: CheckCircle2 },
  failed: { color: 'bg-destructive/20 text-destructive', icon: AlertCircle },
};

// Backward compatibility export (for components that don't need labels or use the hook)
...
/**
 * Hook to get phase configuration with translated labels.
 * Use this in components that display phase labels to users.
 */
export function usePhaseConfig(phase: ExecutionPhase): { label: string; color: string; icon: React.ElementType } {
  const { t } = useTranslation(['terminal']);

  return useMemo(() => {
    const styleConfig = PHASE_STYLE_CONFIG[phase];
    const labelKey = `terminal:phase.${phase}` as const;
    return {
      label: t(labelKey),
