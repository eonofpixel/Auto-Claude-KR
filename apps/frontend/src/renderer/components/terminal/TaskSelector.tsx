...
import { usePhaseConfig } from './types';
...
export function TaskSelector({
  ...
  const phaseConfig = usePhaseConfig(executionPhase);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <span>{t('terminal:task.selectTask')}</span>
          <ChevronDown className="h-2.5 w-2.5 opacity-60" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="start" className="w-56">
        {backlogTasks.length > 0 ? (
          <>
            <div className="px-2 py-1.5 text-xs text-muted-foreground">
              {t('terminal:task.availableTasks')}
            </div>
          ...
          <>
            <div className="px-2 py-1.5 text-xs text-muted-foreground">
              {t('terminal:task.noTasksAvailable')}
            </div>
            {onNewTaskClick ? (
              <DropdownMenuItem
                onClick={(e) => {
              ...
              <div className="px-2 py-1.5 text-xs text-muted-foreground italic">
                {t('terminal:task.createTasksInKanban')}
              </div>
            )}
          </>
        )}
