...
import {
  CHANGELOG_SOURCE_MODE_LABEL_KEYS,
  CHANGELOG_SOURCE_MODE_DESCRIPTION_KEYS
    ...
    <div className="w-80 shrink-0 border-r border-border overflow-y-auto">
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-3">
          <Label className="text-sm font-medium">{t('common:changelog.changelogSource')}</Label>
          <RadioGroup
            value={sourceMode}
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm">
                    {t(CHANGELOG_SOURCE_MODE_LABEL_KEYS['tasks'])}
                  </span>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t(CHANGELOG_SOURCE_MODE_DESCRIPTION_KEYS['tasks'])}
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm">
                    {t(CHANGELOG_SOURCE_MODE_LABEL_KEYS['git-history'])}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t(CHANGELOG_SOURCE_MODE_DESCRIPTION_KEYS['git-history'])}
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm">
                    {t(CHANGELOG_SOURCE_MODE_LABEL_KEYS['branch-diff'])}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t(CHANGELOG_SOURCE_MODE_DESCRIPTION_KEYS['branch-diff'])}
                </p>
              </div>
            </label>
          </RadioGroup>
        ...
        {sourceMode === 'git-history' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('common:changelog.gitHistoryOptions')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* History Type */}
              <div className="space-y-2">
                <Label className="text-xs">{t('common:changelog.historyType')}</Label>
                <Select
                  ...
                  <SelectContent>
                    <SelectItem value="since-version">
                      <div className="flex items-center gap-2">
                        ...
                        {t('common:changelog.sinceVersion')}
                      </div>
                    </SelectItem>
                    <SelectItem value="recent">
                      <div className="flex items-center gap-2">
                        <History className="h-3 w-3" />
                        {t('common:changelog.recentCommits')}
                      </div>
                    </SelectItem>
                    <SelectItem value="since-date">
                      <div className="flex items-center gap-2">
                        <Calendar className="h-3 w-3" />
                        {t('common:changelog.sinceDate')}
                      </div>
                    </SelectItem>
                    <SelectItem value="tag-range">
                      <div className="flex items-center gap-2">
                        <Tag className="h-3 w-3" />
                        {t('common:changelog.tagRange')}
                      </div>
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {gitHistoryType === 'recent' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('common:changelog.numberOfCommits')}</Label>
                  <Input
                    type="number"
                    min={1}
                    max={500}
              ...
              {gitHistoryType === 'since-date' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('common:changelog.sinceDate')}</Label>
                  <Input
                    type="date"
              ...
              {gitHistoryType === 'tag-range' && (
                <>
                  <div className="space-y-2">
                    <Label className="text-xs">{t('common:changelog.fromTag')}</Label>
                    <Select value={gitHistoryFromTag} onValueChange={onGitHistoryFromTagChange}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('common:changelog.placeholders.selectTag')} />
                      </SelectTrigger>
                      <SelectContent>
                        {tags.map((tag) => (
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs">{t('common:changelog.toTagOptional')}</Label>
                    <Select value={gitHistoryToTag || 'HEAD'} onValueChange={(v) => onGitHistoryToTagChange(v === 'HEAD' ? '' : v)}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('common:changelog.placeholders.headLatest')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="HEAD">{t('common:changelog.headLatest')}</SelectItem>
                        {tags.map((tag) => (
                          <SelectItem key={tag.name} value={tag.name}>
                            {tag.name}
                          </SelectItem>
              ...
              {gitHistoryType === 'since-version' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('common:changelog.lastVersion')}</Label>
                  <Select value={gitHistorySinceVersion} onValueChange={onGitHistorySinceVersionChange}>
                    <SelectTrigger>
                      <SelectValue placeholder={t('common:changelog.placeholders.selectVersion')} />
                    </SelectTrigger>
                    <SelectContent>
                      {tags.map((tag) => (
                        <SelectItem key={tag.name} value={tag.name}>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t('common:changelog.sinceVersionDescription')}
                  </p>
                </div>
              )}
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="merge-commits" className="text-xs cursor-pointer">
                  {t('common:changelog.includeMergeCommits')}
                </Label>
              </div>
              ...
              >
                {isLoadingCommits ? (
                  ...
                  <>
                    ...
                    {t('common:changelog.loadCommits')}
                  </>
                )}
              </Button>
            </CardContent>
        ...
        {sourceMode === 'branch-diff' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('common:changelog.branchComparison')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="text-xs">{t('common:changelog.baseBranch')}</Label>
                <Select value={baseBranch} onValueChange={onBaseBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('common:changelog.placeholders.selectBaseBranch')} />
                  </SelectTrigger>
                  <SelectContent>
                      ...
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          ...
                          {branch.name === defaultBranch && (
                            <Badge variant="outline" className="text-xs">{t('common:changelog.default')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {t('common:changelog.baseBranchDescription')}
                </p>
              </div>

              <div className="space-y-2">
                <Label className="text-xs">{t('common:changelog.compareBranch')}</Label>
                <Select value={compareBranch} onValueChange={onCompareBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('common:changelog.placeholders.selectCompareBranch')} />
                  </SelectTrigger>
                  <SelectContent>
                    {localBranches.map((branch) => (
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          {branch.name}
                          {branch.isCurrent && (
                            <Badge variant="secondary" className="text-xs">{t('common:changelog.current')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {t('common:changelog.compareBranchDescription')}
                </p>
              </div>
              ...
              {baseBranch && compareBranch && baseBranch === compareBranch && (
                <div className="flex items-center gap-2 text-destructive text-xs">
                  ...
                  {t('common:changelog.branchesMustBeDifferent')}
                </div>
              )}
