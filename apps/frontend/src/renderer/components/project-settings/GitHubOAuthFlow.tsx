...
export function GitHubOAuthFlow({ onSuccess, onCancel }: GitHubOAuthFlowProps) {
  ...
  const handleAuthTimeout = useCallback(() => {
    ...
    setIsTimeout(true);
    setError(t('githubOAuth.error.timedOutDescription'));
    setStatus('error');
    authTimeoutRef.current = null;
  }, [t]);
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="flex gap-3">
                    <Button onClick={handleOpenGhInstall} className="gap-2">
                      ...
                      {t('githubOAuth.needInstall.installButton')}
                    </Button>
                    <Button variant="outline" onClick={handleRetry}>
                      {t('githubOAuth.needInstall.installedButton')}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 text-sm text-muted-foreground">
                  <p className="font-medium text-foreground mb-2">{t('githubOAuth.needInstall.instructions')}</p>
                  <ul className="space-y-1 list-disc list-inside">
                    <li>{t('githubOAuth.needInstall.macOS')} <code className="px-1.5 py-0.5 bg-muted rounded font-mono text-xs">brew install gh</code></li>
                    <li>{t('githubOAuth.needInstall.windows')} <code className="px-1.5 py-0.5 bg-muted rounded font-mono text-xs">winget install GitHub.cli</code></li>
                    <li>{t('githubOAuth.needInstall.linux')} <a href="https://cli.github.com/" target="_blank" rel="noopener noreferrer" className="text-info hover:underline">cli.github.com</a></li>
                  </ul>
                </div>
              </div>
            </CardContent>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-center">
            <Button onClick={handleStartAuth} size="lg" className="gap-2">
              ...
              {t('githubOAuth.needAuth.authButton')}
            </Button>
          </div>
        </div>
      )}
                ...
                <div className="text-center space-y-4">
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-foreground">
                      {t('githubOAuth.authenticating.oneTimeCode')}
                    </p>
                      ...
                      >
                        {codeCopied ? (
                          <>
                            ...
                            {t('githubOAuth.authenticating.copied')}
                          </>
                        ) : (
                          <>
                            <Copy className="h-4 w-4 mr-1" />
                            {t('githubOAuth.authenticating.copy')}
                          </>
                        )}
                      </Button>
                    </div>
                  ...
                  <div className="text-sm text-muted-foreground space-y-2">
                    <p>
                      {browserOpened
                        ? t('githubOAuth.authenticating.enterCode')
                        : t('githubOAuth.authenticating.copyAndOpen')}
                    </p>
                    {!browserOpened && authUrl && (
                      ...
                      >
                        <ExternalLink className="h-4 w-4" />
                        {t('githubOAuth.authenticating.open')} {authUrl}
                      </Button>
                    )}
                  </div>
                </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-success/80 mt-1">
                  {username ? t('githubOAuth.success.connectedAs', { username }) : t('githubOAuth.success.connected')}
                </p>
              </div>
            </div>
          </CardContent>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className={`text-lg font-medium ${isTimeout ? 'text-warning' : 'text-destructive'}`}>
                    {isTimeout ? t('githubOAuth.error.timedOut') : t('githubOAuth.error.failed')}
                  </h3>
              ...
              <CardContent className="p-5">
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="text-base font-medium text-foreground">
                        {t('githubOAuth.error.manualTitle')}
                      </h3>
                      <p className="text-sm text-muted-foreground mt-1">
                        {t('githubOAuth.error.manualDescription')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    >
                      <ExternalLink className="h-4 w-4" />
                      {t('githubOAuth.error.openInBrowser')}
                    </Button>
                  </div>
