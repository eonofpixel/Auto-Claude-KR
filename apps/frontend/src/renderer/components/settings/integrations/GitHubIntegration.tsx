    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <Label className="font-normal text-foreground">{t('githubIntegration.enableGitHub')}</Label>
          <p className="text-xs text-muted-foreground">
            {t('githubIntegration.enableDescription')}
          </p>
        </div>
        <Switch
      ...
      {envConfig.githubEnabled && (
        <>
              ...
              <div className="rounded-lg border border-success/30 bg-success/10 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-success">{t('githubIntegration.connectedViaGhCli')}</p>
                      {oauthUsername && (
                        <p className="text-xs text-success/80 flex items-center gap-1 mt-0.5">
                          <User className="h-3 w-3" />
                          {t('githubIntegration.authenticatedAs', { username: oauthUsername })}
                        </p>
                      )}
                    </div>
                  </div>
                  <Button
                    ...
                    className="text-xs"
                  >
                    {t('githubIntegration.useDifferentToken')}
                  </Button>
                </div>
              </div>
          ...
          {authMode === 'oauth' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('githubIntegration.authentication')}</Label>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleSwitchToManual}
                >
                  {t('githubIntegration.useManualToken')}
                </Button>
              </div>
              <GitHubOAuthFlow
          ...
          {authMode === 'manual' && (
            <>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-sm font-medium text-foreground">{t('githubIntegration.personalAccessToken')}</Label>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    ...
                    {t('githubIntegration.useOAuthInstead')}
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">
                  {t('githubIntegration.tokenHelp')}{' '}
                  <a
                  ...
                  >
                    {t('githubIntegration.gitHubSettings')}
                  </a>
                </p>
                <PasswordInput
                  value={envConfig.githubToken || ''}
                  onChange={(value) => updateEnvConfig({ githubToken: value })}
                  placeholder={t('githubIntegration.tokenPlaceholder')}
                />
              </div>
...
function RepositoryDropdown({
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <Label className="text-sm font-medium text-foreground">{t('githubIntegration.repository')}</Label>
        <div className="flex items-center gap-2">
          ...
          >
            {t('githubIntegration.enterManually')}
          </Button>
        </div>
      </div>
        ...
        >
          {isLoading ? (
            <span className="flex items-center gap-2 text-muted-foreground">
              ...
              {t('githubIntegration.loadingRepos')}
            </span>
          ) : selectedRepo ? (
            ...
            <span className="text-muted-foreground">{t('githubIntegration.selectRepository')}</span>
          )}
        ...
        {isOpen && !isLoading && (
          <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-64 overflow-hidden">
            ...
            <div className="p-2 border-b border-border">
              <Input
                placeholder={t('githubIntegration.searchRepositories')}
                value={filter}
            ...
            <div className="max-h-48 overflow-y-auto">
              {filteredRepos.length === 0 ? (
                <div className="px-3 py-4 text-sm text-muted-foreground text-center">
                  {filter ? t('githubIntegration.noMatchingRepos') : t('githubIntegration.noReposFound')}
                </div>
              ) : (
...
function RepositoryInput({ value, onChange }: RepositoryInputProps) {
  ...
  return (
    <div className="space-y-2">
      <Label className="text-sm font-medium text-foreground">{t('githubIntegration.repository')}</Label>
      <p className="text-xs text-muted-foreground">
        {t('githubIntegration.repoFormat')}
      </p>
      <Input
        placeholder={t('githubIntegration.repoPlaceholder')}
        value={value}
...
function ConnectionStatus({ isChecking, connectionStatus }: ConnectionStatusProps) {
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/30 p-3">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-foreground">{t('githubIntegration.connectionStatus')}</p>
          <p className="text-xs text-muted-foreground">
            {isChecking ? t('githubIntegration.checking') :
              connectionStatus?.connected
                ? t('githubIntegration.connectedTo', { repo: connectionStatus.repoFullName })
                : connectionStatus?.error || t('githubIntegration.notConnected')}
          </p>
    ...
    <div className="rounded-lg border border-info/30 bg-info/5 p-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-foreground">{t('githubIntegration.issuesAvailable')}</p>
          <p className="text-xs text-muted-foreground mt-1">
            {t('githubIntegration.issuesDescription')}
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="flex items-center justify-between">
      <div className="space-y-0.5">
        <div className="flex items-center gap-2">
          ...
          <Label className="font-normal text-foreground">{t('githubIntegration.autoSync')}</Label>
        </div>
        <p className="text-xs text-muted-foreground pl-6">
          {t('githubIntegration.autoSyncDescription')}
        </p>
      </div>
...
function BranchSelector({
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            ...
            <Label className="text-sm font-medium text-foreground">{t('githubIntegration.defaultBranch')}</Label>
          </div>
          <p className="text-xs text-muted-foreground pl-6">
            {t('githubIntegration.branchDescription')}
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      <div className="relative pl-6">
        ...
        >
          {isLoading ? (
            <span className="flex items-center gap-2 text-muted-foreground">
              <Loader2 className="h-4 w-4 animate-spin" />
              {t('githubIntegration.loadingBranches')}
            </span>
          ) : selectedBranch ? (
            <span className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">{t('githubIntegration.autoDetect')}</span>
          )}
          <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${isOpen ? 'rotate-180' : ''}`} />
        </button>
        ...
        {isOpen && !isLoading && (
          <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-64 overflow-hidden">
            ...
            <div className="p-2 border-b border-border">
              <Input
                placeholder={t('githubIntegration.searchBranches')}
                value={filter}
                onChange={(e) => setFilter(e.target.value)}
                className="h-8 text-sm"
                autoFocus
            ...
            >
              <span className="text-sm text-muted-foreground italic">{t('githubIntegration.autoDetect')}</span>
            </button>
            ...
            <div className="max-h-40 overflow-y-auto border-t border-border">
              {filteredBranches.length === 0 ? (
                <div className="px-3 py-4 text-sm text-muted-foreground text-center">
                  {filter ? t('githubIntegration.noMatchingBranches') : t('githubIntegration.noBranchesFound')}
                </div>
              ) : (
      ...
      {selectedBranch && (
        <p className="text-xs text-muted-foreground pl-6">
          {t('githubIntegration.newTasksBranchFrom')} <code className="px-1 bg-muted rounded">{selectedBranch}</code>
        </p>
      )}
    </div>
  );
