...
export function AgentProfileSettings() {
  ...
  const renderProfileCard = (profile: AgentProfile) => {
    ...
    // Get translated name and description, fallback to original if translation not found
    const profileName = t(`agentProfile.profiles.${profile.id}.name`, { defaultValue: profile.name });
    const profileDescription = t(`agentProfile.profiles.${profile.id}.description`, { defaultValue: profile.description });
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0 pr-6">
            <div className="flex items-center gap-2">
              <h3 className="font-medium text-sm text-foreground">{profileName}</h3>
              {isCustomized && (
                ...
                <div className="flex justify-end">
                  ...
                  >
                    ...
                    {t('agentProfile.resetToProfileDefaults', {
                      profile: t(`agentProfile.profiles.${selectedProfile.id}.name`, { defaultValue: selectedProfile.name })
                    })}
                  </Button>
                </div>
              )}
