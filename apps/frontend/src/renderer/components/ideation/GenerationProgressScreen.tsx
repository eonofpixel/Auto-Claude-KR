        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-lg font-semibold">{t('ideationGeneration.generatingIdeas')}</h2>
              <Badge variant="outline">
                {t('ideationGeneration.progressComplete', { completed: completedCount, total: enabledTypes.length })}
              </Badge>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {showLogs ? t('ideationGeneration.hideLogs') : t('ideationGeneration.showLogs')}
            </Button>
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {isStopping ? t('ideationGeneration.stopping') : t('buttons.stop')}
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t('ideationGeneration.stopGeneration')}</TooltipContent>
            </Tooltip>
          </div>
        </div>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {ideas.length > 0 && (
                    <Badge variant="outline" className="ml-auto">
                      {t('ideationGeneration.ideasCount', { count: ideas.length })}
                    </Badge>
                  )}
                </div>
                ...
                <div className="grid gap-3">
                  ...
                  {state === 'pending' && (
                    <div className="text-sm text-muted-foreground py-2">
                      {t('ideationGeneration.waitingToStart')}
                    </div>
                  )}
                  ...
                  {state === 'failed' && ideas.length === 0 && (
                    <div className="text-sm text-destructive py-2">
                      {t('ideationGeneration.failedToGenerate')}
                    </div>
                  )}
                  ...
                  {state === 'completed' && ideas.length === 0 && (
                    <div className="text-sm text-muted-foreground py-2">
                      {t('ideationGeneration.noIdeasGenerated')}
                    </div>
                  )}
                </div>
              </div>
