...
import {
  IDEATION_TYPE_LABEL_KEYS,
  IDEATION_TYPE_DESCRIPTION_KEYS,
  ...
  return (
    <>
      ...
      <Dialog open={showConfigDialog} onOpenChange={onCloseConfigDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('ideationConfig.title')}</DialogTitle>
            <DialogDescription>
              {t('ideationConfig.description')}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4 max-h-96 overflow-y-auto">
            <div className="space-y-3">
              <h4 className="text-sm font-medium">{t('ideationConfig.ideationTypes')}</h4>
              {ALL_IDEATION_TYPES.map((type) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-sm">{t(IDEATION_TYPE_LABEL_KEYS[type])}</div>
                      <div className="text-xs text-muted-foreground">
                        {t(IDEATION_TYPE_DESCRIPTION_KEYS[type])}
                      </div>
                    </div>
                  </div>
                  <Switch
            ...
            <div className="space-y-3">
              <h4 className="text-sm font-medium">{t('ideationConfig.contextSources')}</h4>
              <div className="flex items-center justify-between">
                <span className="text-sm">{t('ideationConfig.includeRoadmapContext')}</span>
                <Switch
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm">{t('ideationConfig.includeKanbanContext')}</span>
                <Switch
      ...
      <Dialog open={showAddMoreDialog} onOpenChange={onCloseAddMoreDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('ideationConfig.addMoreTitle')}</DialogTitle>
            <DialogDescription>
              {t('ideationConfig.addMoreDescription')}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-3 max-h-96 overflow-y-auto">
            {availableTypesToAdd.length === 0 ? (
              <div className="text-center py-6 text-muted-foreground">
                ...
                <p>{t('ideationConfig.allTypesGenerated')}</p>
                <p className="text-sm mt-1">{t('ideationConfig.useRegenerate')}</p>
              </div>
            ) : (
          ...
          <DialogFooter className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              {typesToAdd.length > 0 && t('ideationConfig.selectedCount', { count: typesToAdd.length })}
            </div>
            <div className="flex gap-2">
              ...
              >
                ...
                {typesToAdd.length > 0
                  ? t('ideationConfig.generateTypes', { count: typesToAdd.length })
                  : t('ideationConfig.generateIdeas')}
              </Button>
            </div>
          </DialogFooter>
        </DialogContent>
