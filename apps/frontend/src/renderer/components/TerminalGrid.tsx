        ...
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground max-w-md">
              ...
              {' '}{t('terminal:grid.shortcutHint', { shortcut: navigator.platform.includes('Mac') ? '⌘+T' : 'Ctrl+T' })}
            </p>
          </div>
        </div>
        ...
        <div className="flex h-10 items-center justify-between border-b border-border bg-card/30 px-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-muted-foreground">
              {t('terminal:grid.terminalCount', { count: terminals.length, max: 12 })}
            </span>
          </div>
          <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    ...
                    {t('terminal:grid.history')}
                    <ChevronDown className="h-3 w-3" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  <div className="px-2 py-1.5 text-xs font-medium text-muted-foreground">
                    {t('terminal:grid.restoreSessionsFrom')}
                  </div>
                  <DropdownMenuSeparator />
                  {sessionDates.map((dateInfo) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t('terminal:grid.sessionCount', { count: dateInfo.sessionCount })}
                      </span>
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
            ...
            {terminals.some((terminal) => terminal.status === 'running' && !terminal.isClaudeMode) && (
              <Button
                variant="outline"
                size="sm"
                className="h-7 text-xs gap-1.5"
