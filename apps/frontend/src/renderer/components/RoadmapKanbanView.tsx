      ...
      <div className="flex items-center justify-between p-4 border-b border-white/5">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="font-semibold text-sm text-foreground">
            {t(ROADMAP_STATUS_LABEL_KEYS[column.id])}
          </h2>
                ...
                >
                  {isOver ? (
                    <>
                      ...
                      <span className="text-sm font-medium text-primary">{t('roadmapKanban.dropHere')}</span>
                    </>
                  ) : (
                    <>
                      <Inbox className="h-6 w-6 text-muted-foreground/50" />
                      <span className="mt-2 text-sm font-medium text-muted-foreground/70">
                        {t('roadmapKanban.noFeatures')}
                      </span>
                      <span className="mt-0.5 text-xs text-muted-foreground/50">
                        {t('roadmapKanban.dragFeaturesHere')}
                      </span>
                    </>
                  )}
                </div>
...
export function RoadmapKanbanView({
  ...
  const getStatusLabelForFeature = (feature: RoadmapFeature) => {
    const labelKey = ROADMAP_STATUS_LABEL_KEYS[feature.status];
    return labelKey ? t(labelKey) : t('roadmapFeature.status.unknown');
  };
