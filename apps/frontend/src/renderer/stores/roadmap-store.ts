...
export async function loadRoadmap(projectId: string): Promise<void> {
  ...
  if (statusResult.success && statusResult.data?.isRunning) {
    ...
    // Use the actual progress from backend if available, otherwise show generic message
    const currentProgress = statusResult.data.currentProgress;
    // Validate phase is a valid value, default to 'analyzing' if not
    const validPhases = ['idle', 'analyzing', 'discovering', 'generating', 'complete', 'error'] as const;
    type ValidPhase = typeof validPhases[number];
    const phase: ValidPhase = currentProgress?.phase && validPhases.includes(currentProgress.phase as ValidPhase)
      ? (currentProgress.phase as ValidPhase)
      : 'analyzing';
    store.setGenerationStatus({
      phase,
      progress: currentProgress?.progress || 0,
      message: currentProgress?.message || 'Roadmap generation in progress...'
    });
  } else {
