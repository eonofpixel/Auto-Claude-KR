...
class TaskMetadataConfig(TypedDict, total=False):
    ...
    model: str
    thinkingLevel: str
    language: str  # Language for AI-generated content (en, ko, fr)
...
# Language code to language name mapping for AI-generated content
...
def get_language(spec_dir: Path) -> str:
    """
    Get the language for AI-generated content from task_metadata.json.
...
def get_language_instruction(spec_dir: Path) -> str:
    """
    Get language instruction string for prompts.
    ...
    """
    language = get_language(spec_dir)
...
**IMPORTANT**: Generate all responses, descriptions, comments, and explanations in **{language_name}**.
This includes:
- Commit messages
- Code comments
- Progress notes
- QA reports and fix requests
- Any human-readable content

Technical terms, code identifiers, and file paths should remain in English.
