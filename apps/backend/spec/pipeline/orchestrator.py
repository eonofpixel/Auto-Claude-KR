...
class SpecOrchestrator:
    ...
    def __init__(
        ...
        language: str = "en",  # Language for AI-generated content (en, ko, fr)
    ):
    ...
    async def _run_agent(
        ...
        # Add language instruction for non-English content
        context_with_language = additional_context
        if self.language and self.language != "en":
            language_name = self.LANGUAGE_NAMES.get(self.language, self.language)
            language_instruction = f"\n\n**IMPORTANT LANGUAGE REQUIREMENT**: Generate ALL content (spec title, description, user stories, acceptance criteria, rationale, etc.) in **{language_name}**. The JSON keys and structure must remain in English, but all human-readable content values must be in {language_name}.\n"
            context_with_language = additional_context + language_instruction

        return await runner.run_agent(
            prompt_file,
            context_with_language,
            interactive,
