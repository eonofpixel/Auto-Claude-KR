...
class AgentExecutor:
    ...
    async def run_agent(
        ...
        if self.language and self.language != "en":
            ...
            prompt += f"\n**IMPORTANT LANGUAGE REQUIREMENT**: Generate ALL content (titles, descriptions, user stories, acceptance criteria, rationale, vision, etc.) in **{language_name}**. The JSON keys and structure must remain in English, but all human-readable content values must be in {language_name}.\n"
            debug_detailed(
                "roadmap_executor",
                f"Added language instruction: {language_name}",
            )
