...
class PRReviewEngine:
    ...
    def _get_language_instruction(self) -> str:
        """Get language instruction for AI-generated content."""
        language = getattr(self.config, "language", "en")
        if language and language != "en":
            language_name = self.LANGUAGE_NAMES.get(language, language)
            return f"\n\n**IMPORTANT LANGUAGE REQUIREMENT**: Write all review comments, summaries, and descriptions in **{language_name}**. The JSON keys must remain in English, but all human-readable content values must be in {language_name}.\n"
        return ""
...
"""
        ...
        full_prompt = pass_prompt + "\n\n---\n\n" + pr_context + self._get_language_instruction()
