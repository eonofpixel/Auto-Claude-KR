...
from phase_config import get_language_instruction
...
def generate_subtask_prompt(
    ...
    # Language instruction first (if non-English)
    language_instruction = get_language_instruction(spec_dir)
...
def generate_planner_prompt(spec_dir: Path, project_dir: Path | None = None) -> str:
    ...
    # Get language instruction if non-English
    language_instruction = get_language_instruction(spec_dir)

    # Build header with language and environment context
    header = language_instruction + generate_environment_context(project_dir, spec_dir)
